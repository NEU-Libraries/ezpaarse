...
function userList() {
  ...
  list.init = function (callback) {
    ...
    mongo.connect(function (err, database) {
      ...
      fs.readFile(list.file, function (err, content) {
        ...
        if (collection) {
          collection.ensureIndex({ username: 1 }, { unique: true }, function (err) {
  ...
  list.getAll = function (callback) {
    if (collection) {
      collection.find({}, { password: 0 }).toArray(callback);
    } else {
  ...
  /**
   * Push a value in an array field
   * @param  {String} username
   * @return {Object} field  field to change OR object of changes
   * @return {Object} value  new value of the field OR callback
   * @return {Object} the user if found, otherwise null
   */
  list.addSettings = function (username, field, value, callback) {
    if (!username || !field || !value) {
      return callback(new Error('missing required parameters'));
    }

    list.get(username, function (err, user) {
      if (err) { return callback(err); }

      if (collection) {
        collection.update({ username: username }, { $push: { field: value } }, function (err, n) {
          if (err) { return callback(err); }

          callback(n ? null : new Error('no changes applied'));
        });
      } else {
        for (var i = this.users.length - 1; i >= 0; i--) {
          var user = this.users[i];
